...
--  Created Date                : Thu Sep 25 17:45:42 ART 2014
--  Modified Date               : Thu Sep 25 17:45:42 ART 2014
--  Created By                  : owb11204_idesa
--  Modified By                 : owb11204_idesa
--  Generated Object Type       : PL/SQL Package
--  Generated Object Name       : "APP_STG_CLIENTE_ORIGEN"
--  Comments                    : 
*****************************************************************************/
...
BEGIN
  ...
  sql_stmt := 'SELECT TABLE_NAME FROM OWB$TEMP_TABLES WHERE PACKAGE_NAME = ''APP_STG_CLIENTE_ORIGEN''';
  ...
  END LOOP;
  CLOSE c1;
  EXECUTE IMMEDIATE 'DELETE FROM OWB$TEMP_TABLES WHERE PACKAGE_NAME = ''APP_STG_CLIENTE_ORIGEN''';
  COMMIT;
EXCEPTION WHEN OTHERS THEN
  DBMS_OUTPUT.PUT_LINE(SQLERRM);
END;  
...
END;
/
CREATE OR REPLACE PACKAGE "APP_STG_CLIENTE_ORIGEN"  AS

-- Map Generation Timestamp
get_generation_date      VARCHAR2(100) := 'Thu Sep 25 17:45:42 ART 2014';
...
get_lob_uoid                  CONSTANT VARCHAR2(40) := '03E7B3C9396014FFE053B200090AD8EB';
get_model_name                CONSTANT VARCHAR2(40) := '"APP_STG_CLIENTE_ORIGEN"';
...
-- Function Main -- Entry point in package "APP_STG_CLIENTE_ORIGEN"
--------------------------------------------------
...
END "APP_STG_CLIENTE_ORIGEN";


/

CREATE OR REPLACE PACKAGE BODY "APP_STG_CLIENTE_ORIGEN" AS

-- Define cursors here so that they have global scope within the package (for debugger)

---------------------------------------------------------------------------
--
-- "EXT_CLIENTE_ORIGEN_c" Cursor declaration 
--
---------------------------------------------------------------------------
CURSOR "EXT_CLIENTE_ORIGEN_c" IS
 SELECT
/* EXT_CLIENTE_ORIGEN.OUTGRP1 */
  "EXT_CLIENTE_ORIGEN"."ID_CLIENTE_ORIGEN" "ID_CLIENTE_ORIGEN",
  "EXT_CLIENTE_ORIGEN"."TX_CLIENTE_ORIGEN" "TX_CLIENTE_ORIGEN"
FROM
  "EXT_CLIENTE_ORIGEN"  "EXT_CLIENTE_ORIGEN"; 

---------------------------------------------------------------------------
--
-- "EXT_CLIENTE_ORIGEN_c$1" Cursor declaration 
--
---------------------------------------------------------------------------
CURSOR "EXT_CLIENTE_ORIGEN_c$1" IS
 SELECT
/* EXT_CLIENTE_ORIGEN.OUTGRP1 */
  "EXT_CLIENTE_ORIGEN"."ID_CLIENTE_ORIGEN" "ID_CLIENTE_ORIGEN",
  "EXT_CLIENTE_ORIGEN"."TX_CLIENTE_ORIGEN" "TX_CLIENTE_ORIGEN"
...
"STG_CLIENTE_ORIGEN_tt_sqlcode"  NUMBER := NULL;
"STG_CLIENTE_ORIGEN_tt_sqlerrm"  VARCHAR2(2000) := NULL;
...
---------------------------------------------------------------------------
-- Function "STG_CLIENTE_ORIGEN_Bat"
...
BEGIN
  ...
  get_abort_procedure := FALSE;
    ...
    batch_auditd_id := OWBSYS.WB_RT_MAPAUDIT.auditd_begin(  -- Template BatchAuditDetailBegin
      ...
      p_step=>0,
      p_name=>'"STG_CLIENTE_ORIGEN_Bat"',
      p_source=>'*',
      p_source_uoid=>'*',
      p_target=>'"STG_CLIENTE_ORIGEN"',
      p_target_uoid=>'03E7B3C9398214FFE053B200090AD8EB',
    ...
    p_parent_operator_uoid=>'03E7B3C9396C14FFE053B200090AD8EB', -- Operator EXT_CLIENTE_ORIGEN
    p_parent_object_name=>'EXT_CLIENTE_ORIGEN',
    p_parent_object_uoid=>'03E7B3C9390814FFE053B200090AD8EB', -- Tabla Externa EXT_CLIENTE_ORIGEN
    p_parent_object_type=>'Tabla Externa',
    p_object_name=>'EXT_CLIENTE_ORIGEN',
    p_object_uoid=>'03E7B3C9390814FFE053B200090AD8EB', -- Tabla Externa EXT_CLIENTE_ORIGEN
    p_object_type=>'Tabla Externa',
    p_location_uoid=>'03C05C514D115B62E053B200090A5929' -- Location DW_STG_LOCATION1
  );
    	get_audit_detail_type_id := OWBSYS.WB_RT_MAPAUDIT.register_audit_detail_type(
    p_rtd=>get_audit_detail_id,
    p_parent_operator_uoid=>'03E7B3C9398214FFE053B200090AD8EB', -- Operator STG_CLIENTE_ORIGEN
  ...
  BEGIN
    ...
    IF NOT "STG_CLIENTE_ORIGEN_St" THEN
      IF get_use_hc THEN
        IF "STG_CLIENTE_ORIGEN_tt_sqlcode" IS NOT NULL THEN
          RAISE_APPLICATION_ERROR(-20016, "STG_CLIENTE_ORIGEN_tt_sqlerrm");
        END IF;
      ELSE
        -- Truncate the target table
        EXECUTE IMMEDIATE 'TRUNCATE TABLE "STG_CLIENTE_ORIGEN"';
      END IF;
    END IF;
    ...
    IF NOT "STG_CLIENTE_ORIGEN_St" THEN
      ...
      INSERT
      INTO
        "STG_CLIENTE_ORIGEN"  "STG_CLIENTE_ORIGEN"
        ("ID_CLIENTE_ORIGEN",
        "TX_CLIENTE_ORIGEN",
        "COD_FECHA_DIARIO",
        "COD_FECHA")
...
/* EXT_CLIENTE_ORIGEN.OUTGRP1 */
  "EXT_CLIENTE_ORIGEN"."ID_CLIENTE_ORIGEN" "ID_CLIENTE_ORIGEN",
  "EXT_CLIENTE_ORIGEN"."TX_CLIENTE_ORIGEN" "TX_CLIENTE_ORIGEN",
  "APP_STG_CLIENTE_ORIGEN"."COD_FECHA_DIARIO"/* ATTRIBUTE P_COD_FECHA_DIARIO.OUTGRP1.COD_FECHA_DIARIO */ "COD_FECHA_DIARIO$1",
  "APP_STG_CLIENTE_ORIGEN"."COD_FECHA"/* ATTRIBUTE P_COD_FECHA.OUTGRP1.COD_FECHA */ "COD_FECHA$1"
FROM
  "EXT_CLIENTE_ORIGEN"  "EXT_CLIENTE_ORIGEN"
        )
      ;
  ...
  last_error_message := SQLERRM;
    ...
    IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
      OWBSYS.WB_RT_MAPAUDIT.error(
        ...
        p_rowkey=>0,
        p_table=>'"STG_CLIENTE_ORIGEN"',
        p_column=>'*',
        p_dstval=>NULL,
        p_stm=>'TRACE 270: ' || batch_action,
        p_sqlerr=>SQLCODE,
        p_sqlerrm=>SQLERRM,
        p_rowid=>NULL
      );
  ...
  END IF;
  RETURN TRUE;
END "STG_CLIENTE_ORIGEN_Bat";



-- Procedure "EXT_CLIENTE_ORIGEN_p" is the entry point for map "EXT_CLIENTE_ORIGEN_p"

PROCEDURE "EXT_CLIENTE_ORIGEN_p"
("CANT_FILAS" IN NUMBER
 DEFAULT NULL,
"COD_FECHA" IN NUMBER
 DEFAULT NULL,
...
get_map_name               CONSTANT VARCHAR2(40) := '"EXT_CLIENTE_ORIGEN_p"';
get_source_name            CONSTANT VARCHAR2(2000) := SUBSTRB('"EXT_CLIENTE_ORIGEN"',0,2000);
get_source_uoid            CONSTANT VARCHAR2(2000) := SUBSTRB('03E7B3C9396C14FFE053B200090AD8EB',0,2000);
...
"STG_CLIENTE_ORIGEN_id" NUMBER(22) := 0;
"STG_CLIENTE_ORIGEN_ins" NUMBER(22) := 0;
"STG_CLIENTE_ORIGEN_upd" NUMBER(22) := 0;
"STG_CLIENTE_ORIGEN_del" NUMBER(22) := 0;
"STG_CLIENTE_ORIGEN_err" NUMBER(22) := 0;
...
"STG_CLIENTE_ORIGEN_srk" OWBSYS.WB_RT_MAPAUDIT.NUMBERLIST;
...
"STG_CLIENTE_ORIGEN_ir"  index_redirect_array;
"SV_STG_CLIENTE_ORIGEN_srk" NUMBER;
"STG_CLIENTE_ORIGEN_new"  BOOLEAN;
"STG_CLIENTE_ORIGEN_nul"  BOOLEAN := FALSE;
...
"EXT_CLIENTE_ORIGEN_si" NUMBER(22) := 0;

"EXT_CLIENTE_ORIGEN_i" NUMBER(22) := 0;
...
TYPE "T_EXT_CLIE_1_ID_CLIEN" IS TABLE OF NUMBER(2) INDEX BY BINARY_INTEGER;
TYPE "T_ROWKEY_EXT_CLIENTE_ORIGEN" IS TABLE OF VARCHAR2(18) INDEX BY BINARY_INTEGER;
TYPE "T_EXT_CLIE_2_TX_CLIEN" IS TABLE OF VARCHAR2(10) INDEX BY BINARY_INTEGER;
...
TYPE "T_STG_CLIE_3_COD_FECH" IS TABLE OF NUMBER(8) INDEX BY BINARY_INTEGER;

-- Bulk: intermediate scalar variables
"SV_EXT_CLIE_1_ID_CLIEN"  NUMBER(2);
"SV_ROWKEY_EXT_CLIENTE_ORIGEN"  VARCHAR2(18);
"SV_EXT_CLIE_2_TX_CLIEN"  VARCHAR2(10);
"SV_STG_CLIE_0_ID_CLIEN"  NUMBER(2);
"SV_STG_CLIE_1_TX_CLIEN"  VARCHAR2(10);
"SV_STG_CLIE_2_COD_FECH"  NUMBER(8);
"SV_STG_CLIE_3_COD_FECH"  NUMBER(8);

-- Bulk: intermediate collection variables
"EXT_CLIE_1_ID_CLIEN" "T_EXT_CLIE_1_ID_CLIEN";
"ROWKEY_EXT_CLIENTE_ORIGEN" "T_ROWKEY_EXT_CLIENTE_ORIGEN";
"EXT_CLIE_2_TX_CLIEN" "T_EXT_CLIE_2_TX_CLIEN";
...
"STG_CLIE_3_COD_FECH" "T_STG_CLIE_3_COD_FECH";
...
---------------------------------------------------------------------------
PROCEDURE "EXT_CLIENTE_ORIGEN_ES"(error_index IN NUMBER) IS
BEGIN
  ...
  OWBSYS.WB_RT_MAPAUDIT.error_source(
    ...
    p_seq=>1,
    p_instance=>1,
    p_table=>SUBSTR('"EXT_CLIENTE_ORIGEN"',0,80),
    p_column=>SUBSTR('ID_CLIENTE_ORIGEN',0,80),
    p_value=>SUBSTRB("EXT_CLIE_1_ID_CLIEN"(error_index),0,2000),
  ...
  OWBSYS.WB_RT_MAPAUDIT.error_source(
    ...
    p_value=>SUBSTRB("EXT_CLIE_2_TX_CLIEN"(error_index),0,2000),
    p_step=>get_step_number,
    p_role=>'S'
    );
  RETURN;
    
  END "EXT_CLIENTE_ORIGEN_ES";

---------------------------------------------------------------------------
-- Procedure "EXT_CLIENTE_ORIGEN_ER" registers error for one erroneous row
---------------------------------------------------------------------------
PROCEDURE "EXT_CLIENTE_ORIGEN_ER"(p_statement IN VARCHAR2, p_column IN VARCHAR2, p_col_value IN VARCHAR2, p_sqlcode IN NUMBER, p_sqlerrm IN VARCHAR2, p_auditd_id IN NUMBER, p_error_index IN NUMBER) IS
...
END IF;
  ...
  IF ( get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE ) AND (get_rowkey_error > 0) THEN
    "EXT_CLIENTE_ORIGEN_ES"(p_error_index);
  END IF;
END "EXT_CLIENTE_ORIGEN_ER";



---------------------------------------------------------------------------
-- Procedure "EXT_CLIENTE_ORIGEN_SU" opens and initializes data source
-- for map "EXT_CLIENTE_ORIGEN_p"
---------------------------------------------------------------------------
PROCEDURE "EXT_CLIENTE_ORIGEN_SU" IS
BEGIN
  IF get_abort THEN
    RETURN;
  END IF;
  IF (NOT "EXT_CLIENTE_ORIGEN_c"%ISOPEN) THEN
    OPEN "EXT_CLIENTE_ORIGEN_c";
  END IF;
  get_read_success := TRUE;
END "EXT_CLIENTE_ORIGEN_SU";

---------------------------------------------------------------------------
-- Procedure "EXT_CLIENTE_ORIGEN_RD" fetches a bulk of rows from
--   the data source for map "EXT_CLIENTE_ORIGEN_p"
---------------------------------------------------------------------------
PROCEDURE "EXT_CLIENTE_ORIGEN_RD" IS
BEGIN
  ...
  BEGIN
    "EXT_CLIE_1_ID_CLIEN".DELETE;
    "EXT_CLIE_2_TX_CLIEN".DELETE;

    FETCH
      "EXT_CLIENTE_ORIGEN_c"
    BULK COLLECT INTO
      "EXT_CLIE_1_ID_CLIEN",
      "EXT_CLIE_2_TX_CLIEN"
    LIMIT get_bulk_size;

    get_total_processed_rowcount := get_total_processed_rowcount + "EXT_CLIE_1_ID_CLIEN".COUNT;

    IF "EXT_CLIENTE_ORIGEN_c"%NOTFOUND AND "EXT_CLIE_1_ID_CLIEN".COUNT = 0 THEN
      RETURN;
    END IF;
    -- register feedback for successful reads
    IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
      get_rowkey := rowkey_counter;
      rowkey_counter := rowkey_counter + "EXT_CLIE_1_ID_CLIEN".COUNT;
    END IF;
    ...
    END IF;
    get_map_selected := get_map_selected + "EXT_CLIE_1_ID_CLIEN".COUNT;
  EXCEPTION
    WHEN OTHERS THEN
        last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
  ...
  END;
END "EXT_CLIENTE_ORIGEN_RD";

---------------------------------------------------------------------------
-- Procedure "EXT_CLIENTE_ORIGEN_DML" does DML for a bulk of rows starting from index si.
---------------------------------------------------------------------------
PROCEDURE "EXT_CLIENTE_ORIGEN_DML"(si NUMBER, firstround BOOLEAN) IS
 "STG_CLIENTE_ORIGEN_ins0" NUMBER := "STG_CLIENTE_ORIGEN_ins";
BEGIN
 IF get_use_hc THEN
  ...
  get_row_status := TRUE;
 END IF;
 IF NOT "STG_CLIENTE_ORIGEN_St" AND (NOT get_use_hc OR get_row_status) 
AND (NOT (get_use_hc AND "STG_CLIENTE_ORIGEN_nul")) THEN
  -- Insert DML for "STG_CLIENTE_ORIGEN"
  normal_action := 'INSERT';
  error_action  := 'INSERT';
  get_target_name := '"STG_CLIENTE_ORIGEN"';
  get_audit_detail_id := "STG_CLIENTE_ORIGEN_id";
  IF get_use_hc AND NOT firstround THEN
    "STG_CLIENTE_ORIGEN_si" := "STG_CLIENTE_ORIGEN_ir"(si);
    IF "STG_CLIENTE_ORIGEN_si" = 0 THEN
      "STG_CLIENTE_ORIGEN_i" := 0;
    ELSE
      "STG_CLIENTE_ORIGEN_i" := "STG_CLIENTE_ORIGEN_si" + 1;
    END IF;
  ELSE
    "STG_CLIENTE_ORIGEN_si" := 1;
  END IF;
  LOOP
    IF NOT get_use_hc THEN
    EXIT WHEN "STG_CLIENTE_ORIGEN_i" <= get_bulk_size 
 AND "EXT_CLIENTE_ORIGEN_c"%FOUND AND NOT get_abort;
    ELSE
      EXIT WHEN "STG_CLIENTE_ORIGEN_si" >= "STG_CLIENTE_ORIGEN_i";
    END IF;
    get_rowid.DELETE;

    BEGIN
      FORALL i IN "STG_CLIENTE_ORIGEN_si".."STG_CLIENTE_ORIGEN_i" - 1
        INSERT 
        INTO
          "STG_CLIENTE_ORIGEN"
          ("STG_CLIENTE_ORIGEN"."ID_CLIENTE_ORIGEN",
          "STG_CLIENTE_ORIGEN"."TX_CLIENTE_ORIGEN",
          "STG_CLIENTE_ORIGEN"."COD_FECHA_DIARIO",
          "STG_CLIENTE_ORIGEN"."COD_FECHA"
          )
        VALUES
          ("STG_CLIE_0_ID_CLIEN"(i),
          "STG_CLIE_1_TX_CLIEN"(i),
          "STG_CLIE_2_COD_FECH"(i),
          "STG_CLIE_3_COD_FECH"(i)
          )
        RETURNING ROWID BULK COLLECT INTO get_rowid;
      error_index := "STG_CLIENTE_ORIGEN_si" + get_rowid.COUNT;
    EXCEPTION
      WHEN OTHERS THEN
          last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
        ...
        error_index := "STG_CLIENTE_ORIGEN_si" + get_rowid.COUNT;
        IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
          error_rowkey := "STG_CLIENTE_ORIGEN_srk"(error_index);
          OWBSYS.WB_RT_MAPAUDIT.error(
            p_rta=>get_runtime_audit_id,
            p_step=>get_step_number,
            p_rtd=>get_audit_detail_id,
          ...
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_CLIENTE_ORIGEN"."ID_CLIENTE_ORIGEN"',0,80),SUBSTRB("STG_CLIE_0_ID_CLIEN"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_CLIENTE_ORIGEN"."TX_CLIENTE_ORIGEN"',0,80),SUBSTRB("STG_CLIE_1_TX_CLIEN"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_CLIENTE_ORIGEN"."COD_FECHA_DIARIO"',0,80),SUBSTRB("STG_CLIE_2_COD_FECH"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_CLIENTE_ORIGEN"."COD_FECHA"',0,80),SUBSTRB("STG_CLIE_3_COD_FECH"(error_index),0,2000));
        ...
        END IF; 
        "STG_CLIENTE_ORIGEN_err" := "STG_CLIENTE_ORIGEN_err" + 1;
        
        IF get_errors + "STG_CLIENTE_ORIGEN_err" > get_max_errors THEN
          get_abort:= TRUE;
        END IF;
    END;
    ...
    IF get_audit_level = AUDIT_COMPLETE THEN
      ...
      rowkey_bulk_index := 1;
      FOR rowkey_index IN "STG_CLIENTE_ORIGEN_si"..error_index - 1 LOOP
        get_rowkey_bulk(rowkey_bulk_index) := "STG_CLIENTE_ORIGEN_srk"(rowkey_index);
...
END IF;
    ...
    "STG_CLIENTE_ORIGEN_ins" := "STG_CLIENTE_ORIGEN_ins" + get_rowid.COUNT;
    "STG_CLIENTE_ORIGEN_si" := error_index + 1;

    IF "STG_CLIENTE_ORIGEN_si" >= "STG_CLIENTE_ORIGEN_i" OR get_abort THEN
      "STG_CLIENTE_ORIGEN_i" := 1;
      EXIT;
    END IF;
  END LOOP;
END IF;
...
END IF;
 ...
 "STG_CLIENTE_ORIGEN_ins" := "STG_CLIENTE_ORIGEN_ins0";
END IF;

END "EXT_CLIENTE_ORIGEN_DML";

---------------------------------------------------------------------------
-- "EXT_CLIENTE_ORIGEN_p" main
---------------------------------------------------------------------------
...
END IF;
  ...
  IF NOT "STG_CLIENTE_ORIGEN_St" THEN
    ...
    IF get_table_function OR (NOT "EXT_CLIENTE_ORIGEN_c"%ISOPEN) THEN
      IF NOT (get_audit_level = AUDIT_NONE) THEN
        IF NOT "STG_CLIENTE_ORIGEN_St" THEN
          "STG_CLIENTE_ORIGEN_id" :=
  ...
  last_error_message := SQLERRM;
            IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
              OWBSYS.WB_RT_MAPAUDIT.error(
                p_rta=>get_runtime_audit_id,
                p_step=>1,
                p_rtd=>"STG_CLIENTE_ORIGEN_id",
                p_rowkey=>0,
                p_table=>'"STG_CLIENTE_ORIGEN"',
                p_column=>'*',
                p_dstval=>NULL,
                p_stm=>'TRACE 276: TRUNCATE',
                p_sqlerr=>SQLCODE,
                p_sqlerrm=>SQLERRM,
                p_rowid=>NULL
              );
...
END IF;
    ...
    "EXT_CLIENTE_ORIGEN_si" := 0;
    "STG_CLIENTE_ORIGEN_i" := 1;
    get_rows_processed := FALSE;

    IF NOT get_abort AND NOT get_abort_procedure THEN
      "EXT_CLIENTE_ORIGEN_SU";

      LOOP
        IF "EXT_CLIENTE_ORIGEN_si" = 0 THEN
          "EXT_CLIENTE_ORIGEN_RD";   -- Fetch data from source
          IF NOT get_read_success THEN
            bulk_count := "EXT_CLIE_1_ID_CLIEN".COUNT - 1;
          ELSE
            bulk_count := "EXT_CLIE_1_ID_CLIEN".COUNT;
          END IF;
                                        IF bulk_commit THEN
                                                bulk_commit_count := 0;
                                                bulk_commit := FALSE;
          ...
          IF get_use_hc THEN
            dml_bsize := 0;
            "STG_CLIENTE_ORIGEN_ir".DELETE;
"STG_CLIENTE_ORIGEN_i" := 1;
          END IF;
        END IF;

        -- Processing:
        "EXT_CLIENTE_ORIGEN_i" := "EXT_CLIENTE_ORIGEN_si";
        BEGIN
          
          LOOP
            EXIT WHEN "STG_CLIENTE_ORIGEN_i" > get_bulk_size OR get_abort OR get_abort_procedure;

            "EXT_CLIENTE_ORIGEN_i" := "EXT_CLIENTE_ORIGEN_i" + 1;
            "EXT_CLIENTE_ORIGEN_si" := "EXT_CLIENTE_ORIGEN_i";
            IF get_use_hc THEN
              get_row_status := TRUE;
                "STG_CLIENTE_ORIGEN_new" := FALSE;
            END IF;

            get_buffer_done(get_buffer_done_index) := 
              ("EXT_CLIENTE_ORIGEN_c"%NOTFOUND AND
               "EXT_CLIENTE_ORIGEN_i" > bulk_count);

            IF (NOT get_buffer_done(get_buffer_done_index)) AND
              "EXT_CLIENTE_ORIGEN_i" > bulk_count THEN
...
get_target_name := '"STG_CLIENTE_ORIGEN"';
            get_audit_detail_id := "STG_CLIENTE_ORIGEN_id";
            IF NOT "STG_CLIENTE_ORIGEN_St" AND NOT get_buffer_done(get_buffer_done_index) THEN
              BEGIN
                get_rows_processed := true; -- Set to indicate that some row data was processed (for debugger)
            		error_stmt := SUBSTRB('"STG_CLIE_0_ID_CLIEN"("STG_CLIENTE_ORIGEN_i") := 
            
            "EXT_CLIE_1_ID_CLIEN"("EXT_CLIENTE_ORIGEN_i");',0,2000);
            error_column := SUBSTRB('"STG_CLIE_0_ID_CLIEN"',0,80);
            
            BEGIN
              error_value := SUBSTRB("EXT_CLIE_1_ID_CLIEN"("EXT_CLIENTE_ORIGEN_i"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            "EXT_CLIE_1_ID_CLIEN"("EXT_CLIENTE_ORIGEN_i");
            
            ELSIF get_row_status THEN
              "SV_STG_CLIE_0_ID_CLIEN" :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_CLIE_1_TX_CLIEN"("STG_CLIENTE_ORIGEN_i") := 
            
            "EXT_CLIE_2_TX_CLIEN"("EXT_CLIENTE_ORIGEN_i");',0,2000);
            error_column := SUBSTRB('"STG_CLIE_1_TX_CLIEN"',0,80);
            
            BEGIN
              error_value := SUBSTRB("EXT_CLIE_2_TX_CLIEN"("EXT_CLIENTE_ORIGEN_i"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            "EXT_CLIE_2_TX_CLIEN"("EXT_CLIENTE_ORIGEN_i");
            
            ELSIF get_row_status THEN
              "SV_STG_CLIE_1_TX_CLIEN" :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_CLIE_2_COD_FECH"("STG_CLIENTE_ORIGEN_i") := 
            
            "APP_STG_CLIENTE_ORIGEN"."COD_FECHA_DIARIO"/* ATTRIBUTE P_COD_FECHA_DIARIO.OUTGRP1.COD_FECHA_DIARIO */;',0,2000);
            error_column := SUBSTRB('"STG_CLIE_2_COD_FECH"',0,80);
            
            BEGIN
              error_value := SUBSTRB("APP_STG_CLIENTE_ORIGEN"."COD_FECHA_DIARIO"/* ATTRIBUTE P_COD_FECHA_DIARIO.OUTGRP1.COD_FECHA_DIARIO */,0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            "APP_STG_CLIENTE_ORIGEN"."COD_FECHA_DIARIO"/* ATTRIBUTE P_COD_FECHA_DIARIO.OUTGRP1.COD_FECHA_DIARIO */;
            
            ELSIF get_row_status THEN
              "SV_STG_CLIE_2_COD_FECH" :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_CLIE_3_COD_FECH"("STG_CLIENTE_ORIGEN_i") := 
            
            "APP_STG_CLIENTE_ORIGEN"."COD_FECHA"/* ATTRIBUTE P_COD_FECHA.OUTGRP1.COD_FECHA */;',0,2000);
            error_column := SUBSTRB('"STG_CLIE_3_COD_FECH"',0,80);
            
            BEGIN
              error_value := SUBSTRB("APP_STG_CLIENTE_ORIGEN"."COD_FECHA"/* ATTRIBUTE P_COD_FECHA.OUTGRP1.COD_FECHA */,0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_CLIE_3_COD_FECH"("STG_CLIENTE_ORIGEN_i") :=
            
            "APP_STG_CLIENTE_ORIGEN"."COD_FECHA"/* ATTRIBUTE P_COD_FECHA.OUTGRP1.COD_FECHA */;
            
            ELSIF get_row_status THEN
              "SV_STG_CLIE_3_COD_FECH" :=
            ...
            END IF;
                ...
                IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
                  IF NOT get_use_hc THEN
                    "STG_CLIENTE_ORIGEN_srk"("STG_CLIENTE_ORIGEN_i") := get_rowkey + "EXT_CLIENTE_ORIGEN_i" - 1;
                  ELSIF get_row_status THEN
                    "SV_STG_CLIENTE_ORIGEN_srk" := get_rowkey + "EXT_CLIENTE_ORIGEN_i" - 1;
                  ELSE
                    NULL;
                  END IF;
                  END IF;
                  IF get_use_hc THEN
                  "STG_CLIENTE_ORIGEN_new" := TRUE;
                ELSE
  ...
  last_error_message := SQLERRM;
                  ...
                  "EXT_CLIENTE_ORIGEN_ER"('TRACE 277: ' || error_stmt, error_column, error_value, SQLCODE, SQLERRM, get_audit_detail_id, "EXT_CLIENTE_ORIGEN_i");
            ...
            IF get_use_hc AND get_row_status AND ("STG_CLIENTE_ORIGEN_new") THEN
              dml_bsize := dml_bsize + 1;
            	IF "STG_CLIENTE_ORIGEN_new" 
            AND (NOT "STG_CLIENTE_ORIGEN_nul") THEN
              "STG_CLIENTE_ORIGEN_ir"(dml_bsize) := "STG_CLIENTE_ORIGEN_i";
            	"STG_CLIE_0_ID_CLIEN"("STG_CLIENTE_ORIGEN_i") := "SV_STG_CLIE_0_ID_CLIEN";
            	"STG_CLIE_1_TX_CLIEN"("STG_CLIENTE_ORIGEN_i") := "SV_STG_CLIE_1_TX_CLIEN";
            	"STG_CLIE_2_COD_FECH"("STG_CLIENTE_ORIGEN_i") := "SV_STG_CLIE_2_COD_FECH";
            	"STG_CLIE_3_COD_FECH"("STG_CLIENTE_ORIGEN_i") := "SV_STG_CLIE_3_COD_FECH";
              "STG_CLIENTE_ORIGEN_srk"("STG_CLIENTE_ORIGEN_i") := "SV_STG_CLIENTE_ORIGEN_srk";
              "STG_CLIENTE_ORIGEN_i" := "STG_CLIENTE_ORIGEN_i" + 1;
            ELSE
              "STG_CLIENTE_ORIGEN_ir"(dml_bsize) := 0;
            END IF;
            END IF;
            
          END LOOP;

          "EXT_CLIENTE_ORIGEN_DML"(1, TRUE);
          IF get_use_hc THEN
            IF NOT get_row_status THEN
              FOR start_index IN 1..dml_bsize LOOP
                "EXT_CLIENTE_ORIGEN_DML"(start_index, FALSE);
              END LOOP;
            ELSE
              COMMIT;
            END IF;
        ...
        EXCEPTION
          WHEN OTHERS THEN
              last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
            "EXT_CLIENTE_ORIGEN_ER"('TRACE 274: ' || error_stmt, '*', NULL, SQLCODE, SQLERRM, NULL, "EXT_CLIENTE_ORIGEN_i");
            get_errors := get_errors + 1;
...
END IF;
  ...
  IF NOT "STG_CLIENTE_ORIGEN_St" AND bulk_commit_count > get_commit_frequency THEN
            IF NOT (get_audit_level = AUDIT_NONE) THEN
              OWBSYS.WB_RT_MAPAUDIT.auditd_progress(
                p_rtd=>"STG_CLIENTE_ORIGEN_id",
                p_sel=>get_map_selected,
                p_ins=>"STG_CLIENTE_ORIGEN_ins",
                p_upd=>"STG_CLIENTE_ORIGEN_upd",
                p_del=>"STG_CLIENTE_ORIGEN_del",
                p_err=>"STG_CLIENTE_ORIGEN_err",
                p_dis=>NULL
              );
            END IF;
            IF NOT get_no_commit THEN
        ...
        cursor_exhausted := "EXT_CLIENTE_ORIGEN_c"%NOTFOUND;
        exit_loop_normal := get_abort OR get_abort_procedure OR (cursor_exhausted AND "EXT_CLIENTE_ORIGEN_i" > bulk_count);
        exit_loop_early := get_rows_processed AND get_bulk_loop_count IS NOT NULL AND "EXT_CLIENTE_ORIGEN_i" >= get_bulk_loop_count;
  ...
  IF NOT "STG_CLIENTE_ORIGEN_St"
    ...
    END IF;
  	get_inserted := get_inserted + "STG_CLIENTE_ORIGEN_ins";
    get_updated  := get_updated  + "STG_CLIENTE_ORIGEN_upd";
    get_deleted  := get_deleted  + "STG_CLIENTE_ORIGEN_del";
    get_errors   := get_errors   + "STG_CLIENTE_ORIGEN_err";
...
END "EXT_CLIENTE_ORIGEN_p";



-- Procedure "EXT_CLIENTE_ORIGEN_t" is the entry point for map "EXT_CLIENTE_ORIGEN_t"

PROCEDURE "EXT_CLIENTE_ORIGEN_t"
("CANT_FILAS" IN NUMBER
 DEFAULT NULL,
"COD_FECHA" IN NUMBER
 DEFAULT NULL,
...
"COD_FECHA_DIARIO" IN NUMBER
 DEFAULT NULL) IS
-- Constants for this map
get_map_name               CONSTANT VARCHAR2(40) := '"EXT_CLIENTE_ORIGEN_t"';
get_source_name            CONSTANT VARCHAR2(2000) := SUBSTRB('"EXT_CLIENTE_ORIGEN"',0,2000);
get_source_uoid            CONSTANT VARCHAR2(2000) := SUBSTRB('03E7B3C9396C14FFE053B200090AD8EB',0,2000);
get_step_number            CONSTANT NUMBER(22) := 1;
...
"EXT_CLIENTE_ORIGEN_si$1" NUMBER(22) := 0;

"EXT_CLIENTE_ORIGEN_i$1" NUMBER(22) := 0;
...
-- Bulk: types for collection variables
TYPE "T_EXT_CLIE_1_ID_CLIEN$1" IS TABLE OF NUMBER(2) INDEX BY BINARY_INTEGER;
TYPE "T_ROWKEY_EXT_CLIENTE_ORIGEN$1" IS TABLE OF VARCHAR2(18) INDEX BY BINARY_INTEGER;
TYPE "T_EXT_CLIE_2_TX_CLIEN$1" IS TABLE OF VARCHAR2(10) INDEX BY BINARY_INTEGER;
...
TYPE "T_STG_CLIE_3_COD_FECH$1" IS TABLE OF NUMBER(8) INDEX BY BINARY_INTEGER;

-- Bulk: intermediate scalar variables
"SV_EXT_CLIE_1_ID_CLIEN$1"  NUMBER(2);
"SV_ROWKEY_EXT_CLIENTE_ORIGEN$1"  VARCHAR2(18);
"SV_EXT_CLIE_2_TX_CLIEN$1"  VARCHAR2(10);
...
"SV_STG_CLIE_3_COD_FECH$1"  NUMBER(8);

-- Bulk: intermediate collection variables
"EXT_CLIE_1_ID_CLIEN$1" "T_EXT_CLIE_1_ID_CLIEN$1";
"ROWKEY_EXT_CLIENTE_ORIGEN$1" "T_ROWKEY_EXT_CLIENTE_ORIGEN$1";
"EXT_CLIE_2_TX_CLIEN$1" "T_EXT_CLIE_2_TX_CLIEN$1";
...
"STG_CLIE_3_COD_FECH$1" "T_STG_CLIE_3_COD_FECH$1";
...
--    RTE_ROWKEY, uniquely identifies a source row within and audited run
---------------------------------------------------------------------------
PROCEDURE "EXT_CLIENTE_ORIGEN_ES$1"(error_index IN NUMBER) IS
BEGIN
  ...
  OWBSYS.WB_RT_MAPAUDIT.error_source(
    ...
    p_table=>SUBSTR('"EXT_CLIENTE_ORIGEN"',0,80),
    p_column=>SUBSTR('ID_CLIENTE_ORIGEN',0,80),
    p_value=>SUBSTRB("EXT_CLIE_1_ID_CLIEN$1"(error_index),0,2000),
    p_step=>get_step_number,
    p_role=>'S'
    );
  OWBSYS.WB_RT_MAPAUDIT.error_source(
    ...
    p_table=>SUBSTR('"EXT_CLIENTE_ORIGEN"',0,80),
    p_column=>SUBSTR('TX_CLIENTE_ORIGEN',0,80),
    p_value=>SUBSTRB("EXT_CLIE_2_TX_CLIEN$1"(error_index),0,2000),
    p_step=>get_step_number,
    p_role=>'S'
    );
  RETURN;
    
  END "EXT_CLIENTE_ORIGEN_ES$1";

---------------------------------------------------------------------------
-- Procedure "EXT_CLIENTE_ORIGEN_ER$1" registers error for one erroneous row
---------------------------------------------------------------------------
PROCEDURE "EXT_CLIENTE_ORIGEN_ER$1"(p_statement IN VARCHAR2, p_column IN VARCHAR2, p_col_value IN VARCHAR2, p_sqlcode IN NUMBER, p_sqlerrm IN VARCHAR2, p_auditd_id IN NUMBER, p_error_index IN NUMBER) IS
l_source_target_name VARCHAR2(80);
BEGIN
  IF p_auditd_id IS NULL THEN
    l_source_target_name := SUBSTR(get_source_name,0,80);
...
END IF;
  ...
  IF ( get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE ) AND (get_rowkey_error > 0) THEN
    "EXT_CLIENTE_ORIGEN_ES$1"(p_error_index);
  END IF;
END "EXT_CLIENTE_ORIGEN_ER$1";



---------------------------------------------------------------------------
-- Procedure "EXT_CLIENTE_ORIGEN_SU$1" opens and initializes data source
-- for map "EXT_CLIENTE_ORIGEN_t"
---------------------------------------------------------------------------
PROCEDURE "EXT_CLIENTE_ORIGEN_SU$1" IS
BEGIN
  IF get_abort THEN
    RETURN;
  END IF;
  IF (NOT "EXT_CLIENTE_ORIGEN_c$1"%ISOPEN) THEN
    OPEN "EXT_CLIENTE_ORIGEN_c$1";
  END IF;
  get_read_success := TRUE;
END "EXT_CLIENTE_ORIGEN_SU$1";

---------------------------------------------------------------------------
-- Procedure "EXT_CLIENTE_ORIGEN_RD$1" fetches a bulk of rows from
--   the data source for map "EXT_CLIENTE_ORIGEN_t"
---------------------------------------------------------------------------
PROCEDURE "EXT_CLIENTE_ORIGEN_RD$1" IS
BEGIN
  IF NOT get_read_success THEN
    get_abort := TRUE;
    IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
  ...
  BEGIN
    "EXT_CLIE_1_ID_CLIEN$1".DELETE;
    "EXT_CLIE_2_TX_CLIEN$1".DELETE;

    FETCH
      "EXT_CLIENTE_ORIGEN_c$1"
    BULK COLLECT INTO
      "EXT_CLIE_1_ID_CLIEN$1",
      "EXT_CLIE_2_TX_CLIEN$1"
    LIMIT get_bulk_size;

    get_total_processed_rowcount := get_total_processed_rowcount + "EXT_CLIE_1_ID_CLIEN$1".COUNT;

    IF "EXT_CLIENTE_ORIGEN_c$1"%NOTFOUND AND "EXT_CLIE_1_ID_CLIEN$1".COUNT = 0 THEN
      RETURN;
    END IF;
    -- register feedback for successful reads
    IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
      get_rowkey := rowkey_counter;
      rowkey_counter := rowkey_counter + "EXT_CLIE_1_ID_CLIEN$1".COUNT;
    END IF;
    ...
    END IF;
    get_map_selected := get_map_selected + "EXT_CLIE_1_ID_CLIEN$1".COUNT;
  EXCEPTION
    WHEN OTHERS THEN
        last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
      ...
      IF get_errors > get_max_errors THEN get_abort := TRUE; END IF;
  END;
END "EXT_CLIENTE_ORIGEN_RD$1";

---------------------------------------------------------------------------
-- Procedure "EXT_CLIENTE_ORIGEN_DML$1" does DML for a bulk of rows starting from index si.
---------------------------------------------------------------------------
PROCEDURE "EXT_CLIENTE_ORIGEN_DML$1"(si NUMBER, firstround BOOLEAN) IS
 "STG_CLIENTE_ORIGEN_ins0" NUMBER := "STG_CLIENTE_ORIGEN_ins";
BEGIN
 IF get_use_hc THEN
  IF firstround AND NOT get_row_status THEN
...
AND (NOT (get_use_hc AND "STG_CLIENTE_ORIGEN_nul")) THEN
  ...
  LOOP
    IF NOT get_use_hc THEN
    EXIT WHEN "STG_CLIENTE_ORIGEN_i" <= get_bulk_size 
 AND "EXT_CLIENTE_ORIGEN_c$1"%FOUND AND NOT get_abort;
    ELSE
      EXIT WHEN "STG_CLIENTE_ORIGEN_si" >= "STG_CLIENTE_ORIGEN_i";
    END IF;
    get_rowid.DELETE;
    ...
    BEGIN
      FORALL i IN "STG_CLIENTE_ORIGEN_si".."STG_CLIENTE_ORIGEN_i" - 1
        ...
        VALUES
          ("STG_CLIE_0_ID_CLIEN$1"(i),
          "STG_CLIE_1_TX_CLIEN$1"(i),
          "STG_CLIE_2_COD_FECH$1"(i),
          "STG_CLIE_3_COD_FECH$1"(i)
          )
        RETURNING ROWID BULK COLLECT INTO get_rowid;
      error_index := "STG_CLIENTE_ORIGEN_si" + get_rowid.COUNT;
    EXCEPTION
  ...
  last_error_message := SQLERRM;
        ...
        IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
          ...
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_CLIENTE_ORIGEN"."ID_CLIENTE_ORIGEN"',0,80),SUBSTRB("STG_CLIE_0_ID_CLIEN$1"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_CLIENTE_ORIGEN"."TX_CLIENTE_ORIGEN"',0,80),SUBSTRB("STG_CLIE_1_TX_CLIEN$1"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_CLIENTE_ORIGEN"."COD_FECHA_DIARIO"',0,80),SUBSTRB("STG_CLIE_2_COD_FECH$1"(error_index),0,2000));
          Main_ES(get_step_number,error_rowkey,get_target_name,SUBSTRB('"STG_CLIENTE_ORIGEN"."COD_FECHA"',0,80),SUBSTRB("STG_CLIE_3_COD_FECH$1"(error_index),0,2000));
...
END "EXT_CLIENTE_ORIGEN_DML$1";

---------------------------------------------------------------------------
-- "EXT_CLIENTE_ORIGEN_t" main
---------------------------------------------------------------------------
...
END IF;
  ...
  IF NOT "STG_CLIENTE_ORIGEN_St" THEN
    ...
    -- For table function (parallel row mode) operation, the cursor will already be
    -- open when the procedure is called, so execute the initialization.
    IF get_table_function OR (NOT "EXT_CLIENTE_ORIGEN_c$1"%ISOPEN) THEN
      IF NOT (get_audit_level = AUDIT_NONE) THEN
        IF NOT "STG_CLIENTE_ORIGEN_St" THEN
          "STG_CLIENTE_ORIGEN_id" :=
            OWBSYS.WB_RT_MAPAUDIT.auditd_begin(  -- Template AuditDetailBegin
...
END IF;
    ...
    -- Initialize internal loop index variables
    "EXT_CLIENTE_ORIGEN_si$1" := 0;
    "STG_CLIENTE_ORIGEN_i" := 1;
    get_rows_processed := FALSE;

    IF NOT get_abort AND NOT get_abort_procedure THEN
      "EXT_CLIENTE_ORIGEN_SU$1";

      LOOP
        IF "EXT_CLIENTE_ORIGEN_si$1" = 0 THEN
          "EXT_CLIENTE_ORIGEN_RD$1";   -- Fetch data from source
          IF NOT get_read_success THEN
            bulk_count := "EXT_CLIE_1_ID_CLIEN$1".COUNT - 1;
          ELSE
            bulk_count := "EXT_CLIE_1_ID_CLIEN$1".COUNT;
          END IF;
                                        IF bulk_commit THEN
                                                bulk_commit_count := 0;
                                                bulk_commit := FALSE;
...
"STG_CLIENTE_ORIGEN_i" := 1;
        ...
        -- Processing:
        "EXT_CLIENTE_ORIGEN_i$1" := "EXT_CLIENTE_ORIGEN_si$1";
        BEGIN
          
          LOOP
            EXIT WHEN "STG_CLIENTE_ORIGEN_i" > get_bulk_size OR get_abort OR get_abort_procedure;

            "EXT_CLIENTE_ORIGEN_i$1" := "EXT_CLIENTE_ORIGEN_i$1" + 1;
            "EXT_CLIENTE_ORIGEN_si$1" := "EXT_CLIENTE_ORIGEN_i$1";
            IF get_use_hc THEN
              get_row_status := TRUE;
                "STG_CLIENTE_ORIGEN_new" := FALSE;
            END IF;
            ...
            get_buffer_done(get_buffer_done_index) := 
              ("EXT_CLIENTE_ORIGEN_c$1"%NOTFOUND AND
               "EXT_CLIENTE_ORIGEN_i$1" > bulk_count);

            IF (NOT get_buffer_done(get_buffer_done_index)) AND
              "EXT_CLIENTE_ORIGEN_i$1" > bulk_count THEN
...
get_target_name := '"STG_CLIENTE_ORIGEN"';
            ...
            IF NOT "STG_CLIENTE_ORIGEN_St" AND NOT get_buffer_done(get_buffer_done_index) THEN
              BEGIN
                get_rows_processed := true; -- Set to indicate that some row data was processed (for debugger)
            		error_stmt := SUBSTRB('"STG_CLIE_0_ID_CLIEN$1"("STG_CLIENTE_ORIGEN_i") := 
            
            "EXT_CLIE_1_ID_CLIEN$1"("EXT_CLIENTE_ORIGEN_i$1");',0,2000);
            error_column := SUBSTRB('"STG_CLIE_0_ID_CLIEN$1"',0,80);
            
            BEGIN
              error_value := SUBSTRB("EXT_CLIE_1_ID_CLIEN$1"("EXT_CLIENTE_ORIGEN_i$1"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_CLIE_0_ID_CLIEN$1"("STG_CLIENTE_ORIGEN_i") :=
            
            "EXT_CLIE_1_ID_CLIEN$1"("EXT_CLIENTE_ORIGEN_i$1");
            
            ELSIF get_row_status THEN
              "SV_STG_CLIE_0_ID_CLIEN$1" :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_CLIE_1_TX_CLIEN$1"("STG_CLIENTE_ORIGEN_i") := 
            
            "EXT_CLIE_2_TX_CLIEN$1"("EXT_CLIENTE_ORIGEN_i$1");',0,2000);
            error_column := SUBSTRB('"STG_CLIE_1_TX_CLIEN$1"',0,80);
            
            BEGIN
              error_value := SUBSTRB("EXT_CLIE_2_TX_CLIEN$1"("EXT_CLIENTE_ORIGEN_i$1"),0,2000);
            EXCEPTION 
              WHEN OTHERS THEN
                error_value := '*';
            END;
            ...
            IF NOT get_use_hc THEN
              "STG_CLIE_1_TX_CLIEN$1"("STG_CLIENTE_ORIGEN_i") :=
            
            "EXT_CLIE_2_TX_CLIEN$1"("EXT_CLIENTE_ORIGEN_i$1");
            
            ELSIF get_row_status THEN
              "SV_STG_CLIE_1_TX_CLIEN$1" :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_CLIE_2_COD_FECH$1"("STG_CLIENTE_ORIGEN_i") := 
            
            "APP_STG_CLIENTE_ORIGEN"."COD_FECHA_DIARIO"/* ATTRIBUTE P_COD_FECHA_DIARIO.OUTGRP1.COD_FECHA_DIARIO */;',0,2000);
            error_column := SUBSTRB('"STG_CLIE_2_COD_FECH$1"',0,80);
            ...
            IF NOT get_use_hc THEN
              "STG_CLIE_2_COD_FECH$1"("STG_CLIENTE_ORIGEN_i") :=
            
            "APP_STG_CLIENTE_ORIGEN"."COD_FECHA_DIARIO"/* ATTRIBUTE P_COD_FECHA_DIARIO.OUTGRP1.COD_FECHA_DIARIO */;
            
            ELSIF get_row_status THEN
              "SV_STG_CLIE_2_COD_FECH$1" :=
            ...
            END IF;
            		...
            		error_stmt := SUBSTRB('"STG_CLIE_3_COD_FECH$1"("STG_CLIENTE_ORIGEN_i") := 
            
            "APP_STG_CLIENTE_ORIGEN"."COD_FECHA"/* ATTRIBUTE P_COD_FECHA.OUTGRP1.COD_FECHA */;',0,2000);
            error_column := SUBSTRB('"STG_CLIE_3_COD_FECH$1"',0,80);
            ...
            IF NOT get_use_hc THEN
              "STG_CLIE_3_COD_FECH$1"("STG_CLIENTE_ORIGEN_i") :=
            
            "APP_STG_CLIENTE_ORIGEN"."COD_FECHA"/* ATTRIBUTE P_COD_FECHA.OUTGRP1.COD_FECHA */;
            
            ELSIF get_row_status THEN
              "SV_STG_CLIE_3_COD_FECH$1" :=
            ...
            END IF;
                ...
                IF get_audit_level = AUDIT_ERROR_DETAILS OR get_audit_level = AUDIT_COMPLETE THEN
                  IF NOT get_use_hc THEN
                    "STG_CLIENTE_ORIGEN_srk"("STG_CLIENTE_ORIGEN_i") := get_rowkey + "EXT_CLIENTE_ORIGEN_i$1" - 1;
                  ELSIF get_row_status THEN
                    "SV_STG_CLIENTE_ORIGEN_srk" := get_rowkey + "EXT_CLIENTE_ORIGEN_i$1" - 1;
                  ELSE
                    NULL;
                  END IF;
                  END IF;
  ...
  last_error_message := SQLERRM;
                  ...
                  "EXT_CLIENTE_ORIGEN_ER$1"('TRACE 284: ' || error_stmt, error_column, error_value, SQLCODE, SQLERRM, get_audit_detail_id, "EXT_CLIENTE_ORIGEN_i$1");
            ...
            AND (NOT "STG_CLIENTE_ORIGEN_nul") THEN
              "STG_CLIENTE_ORIGEN_ir"(dml_bsize) := "STG_CLIENTE_ORIGEN_i";
            	"STG_CLIE_0_ID_CLIEN$1"("STG_CLIENTE_ORIGEN_i") := "SV_STG_CLIE_0_ID_CLIEN$1";
            	"STG_CLIE_1_TX_CLIEN$1"("STG_CLIENTE_ORIGEN_i") := "SV_STG_CLIE_1_TX_CLIEN$1";
            	"STG_CLIE_2_COD_FECH$1"("STG_CLIENTE_ORIGEN_i") := "SV_STG_CLIE_2_COD_FECH$1";
            	"STG_CLIE_3_COD_FECH$1"("STG_CLIENTE_ORIGEN_i") := "SV_STG_CLIE_3_COD_FECH$1";
              "STG_CLIENTE_ORIGEN_srk"("STG_CLIENTE_ORIGEN_i") := "SV_STG_CLIENTE_ORIGEN_srk";
              "STG_CLIENTE_ORIGEN_i" := "STG_CLIENTE_ORIGEN_i" + 1;
            ELSE
              "STG_CLIENTE_ORIGEN_ir"(dml_bsize) := 0;
          ...
          "EXT_CLIENTE_ORIGEN_DML$1"(1, TRUE);
          IF get_use_hc THEN
            IF NOT get_row_status THEN
              FOR start_index IN 1..dml_bsize LOOP
                "EXT_CLIENTE_ORIGEN_DML$1"(start_index, FALSE);
              END LOOP;
            ELSE
              COMMIT;
            END IF;
        ...
        EXCEPTION
          WHEN OTHERS THEN
              last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
            "EXT_CLIENTE_ORIGEN_ER$1"('TRACE 281: ' || error_stmt, '*', NULL, SQLCODE, SQLERRM, NULL, "EXT_CLIENTE_ORIGEN_i$1");
            get_errors := get_errors + 1;
            IF get_errors > get_max_errors THEN  
  get_abort := TRUE;
END IF;
  ...
  IF NOT "STG_CLIENTE_ORIGEN_St" AND bulk_commit_count > get_commit_frequency THEN
        ...
        cursor_exhausted := "EXT_CLIENTE_ORIGEN_c$1"%NOTFOUND;
        exit_loop_normal := get_abort OR get_abort_procedure OR (cursor_exhausted AND "EXT_CLIENTE_ORIGEN_i$1" > bulk_count);
        exit_loop_early := get_rows_processed AND get_bulk_loop_count IS NOT NULL AND "EXT_CLIENTE_ORIGEN_i$1" >= get_bulk_loop_count;
        get_close_cursor := get_abort OR get_abort_procedure OR cursor_exhausted;
        EXIT WHEN exit_loop_normal OR exit_loop_early;
...
"APP_STG_CLIENTE_ORIGEN"."CANT_FILAS" := "APP_STG_CLIENTE_ORIGEN".Initialize."CANT_FILAS";
"APP_STG_CLIENTE_ORIGEN"."COD_FECHA" := "APP_STG_CLIENTE_ORIGEN".Initialize."COD_FECHA";
"APP_STG_CLIENTE_ORIGEN"."TABLE_NAME" := "APP_STG_CLIENTE_ORIGEN".Initialize."TABLE_NAME";
"APP_STG_CLIENTE_ORIGEN"."COD_FECHA_DIARIO" := "APP_STG_CLIENTE_ORIGEN".Initialize."COD_FECHA_DIARIO";
  ...
  IF NOT (get_audit_level = AUDIT_NONE) THEN
    get_runtime_audit_id := OWBSYS.WB_RT_MAPAUDIT.audit_begin(  -- Template AuditBegin
      ...
      p_parent=>NULL,
      p_source=>'"EXT_CLIENTE_ORIGEN"',
      p_source_uoid=>'03E7B3C9396C14FFE053B200090AD8EB',
      p_target=>'"STG_CLIENTE_ORIGEN"',
      p_target_uoid=>'03E7B3C9398214FFE053B200090AD8EB',      p_info=>NULL,
...
BEGIN
  ...
  "PACK_STG_UTILS"."FN_TRAILER_CHECK"(("APP_STG_CLIENTE_ORIGEN"."TABLE_NAME"/* ATTRIBUTE P_TABLE_NAME.OUTGRP1.TABLE_NAME */),("APP_STG_CLIENTE_ORIGEN"."CANT_FILAS"/* ATTRIBUTE P_CANT_FILAS.OUTGRP1.CANT_FILAS */))/* OPERATOR FN_TRAILER_CHECK */;
...
EXCEPTION
  WHEN OTHERS THEN
      last_error_number  := SQLCODE;
  last_error_message := SQLERRM;
    "STG_CLIENTE_ORIGEN_tt_sqlcode" := SQLCODE;
    "STG_CLIENTE_ORIGEN_tt_sqlerrm" := SQLERRM;
END;
...
BEGIN
  ...
  END IF;
        ...
        AND    uo.object_name = 'APP_STG_CLIENTE_ORIGEN'
        AND    uo.object_id = ao.object_id;
 
        OWBSYS.wb_rt_mapaudit_util.set_schema_workspace(x_schema);
    
  Initialize("CANT_FILAS", "COD_FECHA", "TABLE_NAME", "COD_FECHA_DIARIO", 
  p_env);
  ...
  END IF;
   ...
   IF get_operating_mode = MODE_SET THEN
    ...
    IF NOT get_use_hc OR get_batch_status THEN
        "STG_CLIENTE_ORIGEN_St" := "STG_CLIENTE_ORIGEN_Bat"("CANT_FILAS", "COD_FECHA", "TABLE_NAME", "COD_FECHA_DIARIO");
        get_batch_status := get_batch_status AND "STG_CLIENTE_ORIGEN_St";
      END IF;
   ...
   "EXT_CLIENTE_ORIGEN_p"("CANT_FILAS", "COD_FECHA", "TABLE_NAME", "COD_FECHA_DIARIO");
   END IF;
  ...
  "EXT_CLIENTE_ORIGEN_t"("CANT_FILAS", "COD_FECHA", "TABLE_NAME", "COD_FECHA_DIARIO");
  ...
  END IF;
   ...
   Finalize("CANT_FILAS", "COD_FECHA", "TABLE_NAME", "COD_FECHA_DIARIO", 
  p_env);
...
BEGIN
  ...
  IF get_job_audit THEN 
     OWBSYS.wb_rt_mapaudit_util.premap('APP_STG_CLIENTE_ORIGEN', x_schema, x_audit_id, x_object_id);
  ELSE
    OWBSYS.wb_rt_mapaudit_util.set_schema_workspace(x_schema);
  END IF;
...
-- OWBSYS.wb_rt_mapaudit_util.register_custom_param(x_audit_id,
  ...
  x_return_code := NULL;
  BEGIN
      x_result := Main( "CANT_FILAS", "COD_FECHA", "TABLE_NAME", "COD_FECHA_DIARIO",
      x_env);
  EXCEPTION
    WHEN OTHERS THEN
      x_result := 1;
...
BEGIN
BEGIN
  IF "EXT_CLIENTE_ORIGEN_c"%ISOPEN THEN
    CLOSE "EXT_CLIENTE_ORIGEN_c";
  END IF;
EXCEPTION
  WHEN OTHERS THEN
    NULL;    
END;BEGIN
  IF "EXT_CLIENTE_ORIGEN_c$1"%ISOPEN THEN
    CLOSE "EXT_CLIENTE_ORIGEN_c$1";
  END IF;
EXCEPTION
  WHEN OTHERS THEN
